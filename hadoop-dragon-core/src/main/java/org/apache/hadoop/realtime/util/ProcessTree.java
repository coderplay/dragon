...
public class ProcessTree {
   ...
   * 
   * @param pid process id of the root process of the subtree of processes to be
   *          killed
   * @param sleeptimeBeforeSigkill The time to wait before sending SIGKILL after
   *          sending SIGTERM
   * @param isProcessGroup pid is a process group leader or not
   * @param inBackground Process is to be killed in the back ground with a
   *          separate thread
   */
   ...
   * @param sleeptimeBeforeSigkill The time to wait before sending SIGKILL after
   *          sending SIGTERM
   * @param inBackground Process group is to be killed in the back ground with a
   *          separate thread
   */
   ...
   * @param signalName the human-readable description of the signal (for
   *          logging).
   */
  private static void sendSignal(String pid, int signalNum, String signalName) {
    ...
    try {
      ...
      if (pid.startsWith("-")) {
        ...
        LOG.info("Signaling process " + pid + " with " + signalName
   ...
   *          sending SIGTERM
   */
  private static void sigKillInCurrentThread(String pid,
      boolean isProcessGroup, long sleepTimeBeforeSigKill) {
  ...
  private static void sigKill(String pid, boolean isProcessGroup,
    ...
    if (inBackground) { // use a separate thread for killing
      SigKillThread sigKillThread =
          new SigKillThread(pid, isProcessGroup, sleeptimeBeforeSigkill);
   ...
   * ordering all processes in the group to send their stack dump to stdout.
   * 
   * @param pgrpId process group id
   */
   ...
   * 
   * This method assumes that isAlive is called on a pid that was alive not too
   * long ago, and hence assumes no chance of pid-wrapping-around.
   * 
   * @param pgrpId process group id
