...
import org.apache.hadoop.realtime.util.ProcessTree;
...
public class TaskLog {
  ...
  public static String getMRv2LogDir() {
    return System.getProperty(DragonJobConfig.TASK_LOG_DIR);
  }
  ...
  static String obtainLogDirOwner(TaskAttemptId taskid) throws IOException {
    ...
    Path jobLogDir = new Path(getJobDir(taskid.getTaskId().getJobId()).getAbsolutePath());
  ...
  static File getAttemptDir(TaskAttemptId taskid, boolean isCleanup) {
    ...
    return new File(getJobDir(taskid.getTaskId().getJobId()), taskid + cleanupSuffix);
  }
  ...
  public static long getTaskLogLength(Configuration conf) {
    return conf.getLong(DragonJobConfig.TASK_USERLOG_LIMIT, 0) * 1024;
  }
