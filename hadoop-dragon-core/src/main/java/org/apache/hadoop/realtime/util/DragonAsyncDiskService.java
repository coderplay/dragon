...
/**
 * This class is a container of multiple thread pools, each for a volume, so
 * that we can schedule async disk operations easily.
 * 
 * Examples of async disk operations are deletion of files. We can move the
 * files to a "toBeDeleted" folder before asychronously deleting it, to make
 * sure the caller can run it faster.
 * 
 * Users should not write files into the "toBeDeleted" folder, otherwise the
 * files can be gone any time we restart the MRAsyncDiskService.
 * 
 * This class also contains all operations that will be performed by the thread
 * pools.
 */
@InterfaceAudience.Private
@InterfaceStability.Unstable
public class DragonAsyncDiskService {

	public static final Log LOG = LogFactory.getLog(DragonAsyncDiskService.class);
	...
	/**
	 * Create a AsyncDiskServices with a set of volumes (specified by their root
	 * directories).
	 * 
	 ...
	 * @param nonCanonicalVols
	 *          The roots of the file system volumes, which may be absolte paths,
	 *          or paths relative to the ${user.dir} system property ("cwd").
	 */
	public DragonAsyncDiskService(FileSystem localFileSystem,
		...
		for (int v = 0; v < volumes.length; v++) {
			...
			if (!localFileSystem.mkdirs(absoluteSubdir)) {
				...
				LOG.warn("Cannot create " + TOBEDELETED + " in " + volumes[v]
				    + ". Ignored.");
			}
		}
	 ...
	 */
	public DragonAsyncDiskService(Configuration conf) throws IOException {
		this(FileSystem.getLocal(conf), conf.getTrimmed(DragonConfig.LOCAL_DIR));
	}
	...
	class DeleteTask implements Runnable {
		 ...
		 *          The name after the move, relative to volume root, containing
		 *          TOBEDELETED.
		 */
	...
	/**
	 * Move the path name on one volume to a temporary location and then delete
	 * them.
	 * 
	 * This functions returns when the moves are done, but not necessarily all
	 * deletions are done. This is usually good enough because applications won't
	 * see the path name under the old name anyway after the move.
	 * 
	 * @param volume
	 *          The disk volume
	 * @param pathName
	...
	public boolean moveAndDeleteRelativePath(String volume, String pathName)
		...
		String newPathName = format.format(new Date()) + "_"
	...
	/**
	 * Move the path name on each volume to a temporary location and then delete
	 * them.
	 * 
	 * This functions returns when the moves are done, but not necessarily all
	 * deletions are done. This is usually good enough because applications won't
	 ...
	 * @throws IOException
	 *           If any of the move failed
	 * @return false If any of the target pathName did not exist, note that the
	 *         operation is still done on all volumes.
	 */
	...
	/**
	 * Move all files/directories inside volume into TOBEDELETED, and then delete
	 * them. The TOBEDELETED directory itself is ignored.
	 */
	...
	private String normalizePath(String path) {
		return (new Path(path)).makeQualified(this.localFileSystem).toUri()
		    .getPath();
	}
	 ...
	 * 
	 * Note that if there is no volume that contains this path, the path will stay
	 * as it is, and the function will return false.
	 * 
	 * This functions returns when the moves are done, but not necessarily all
	 * deletions are done. This is usually good enough because applications won't
	 * see the path name under the old name anyway after the move.
